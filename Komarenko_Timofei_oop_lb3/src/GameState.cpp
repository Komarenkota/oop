...
std::ostream& operator<< (std::ostream &os, GameState& gameState) {
    if (!os) throw std::runtime_error("Не удалось открыть файл для записи!");
...
unsigned int GameState::calculateChecksum(const std::string& data) {
    const unsigned int base = 31; // Основание (можно выбрать другое простое число)
    const unsigned int mod = 1e9 + 7; // Большое простое число для предотвращения переполнения
    unsigned int hash = 0;
    unsigned int power = 1; // base^0 = 1

    for (char c : data) {
        hash = (hash + (static_cast<unsigned int>(c) * power) % mod) % mod;
        power = (power * base) % mod; // Увеличиваем степень основания
    }
